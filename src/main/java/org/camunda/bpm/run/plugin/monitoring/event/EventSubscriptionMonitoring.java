...
//            MultiGaugeData data = map.get(groupByKey);
            ...
            switch (eventSubscription.getEventType()) {
                case ("message"):
                    data.getGaugesValues().merge(Meters.MESSAGE.getMeterName(), Long.valueOf(1), Long::sum);
                    break;
                case ("signal"):
                    data.getGaugesValues().merge(Meters.SIGNAL.getMeterName(), Long.valueOf(1), Long::sum);
                    break;
                case ("compensate"):
                    data.getGaugesValues().merge(Meters.COMPENSATE.getMeterName(), Long.valueOf(1), Long::sum);
                    break;
                case ("conditional"):
                    data.getGaugesValues().merge(Meters.CONDITIONAL.getMeterName(), Long.valueOf(1), Long::sum);
                    break;
            }
        }
